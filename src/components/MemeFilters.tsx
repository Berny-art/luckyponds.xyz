...
export default function MemeFilters({ disabled }: { disabled: boolean }) {
	const { searchInput, setSearchInput, selectedLayers, toggleLayer, reset } = useMemeStore();

	return (
		<div className="flex flex-col items-center w-full gap-6">
            <div>
                <h1 className="text-primary-200 uppercase text-lg font-bold w-full">Meme Generator</h1>
                <p className="font-normal text-primary-200">Find your frog and select from various meme layers to make it stand-out!</p>
            </div>
			<Input
				...
				{Object.entries(memeLayerCategories).map(([category, layers]) => (
					<div key={category} className="flex flex-col gap-2">
						...
						<div className="flex flex-wrap gap-2">
							{layers.map((layer) => {
								const isSelected = selectedLayers[category as MemeLayerCategory] === layer;
								return (
									<Button
										key={layer}
										disabled={disabled}
										onClick={() => toggleLayer(category as MemeLayerCategory, layer)}
